...
class minAgeSparcle():
    ...
    def __init__(self,
        ...
        self.alc_percent_min = alc_percent_min
        self.alc_percent_max = alc_percent_max
    ...
    def db_selecter(self, query, params, max_retries=3, retry_delay=1):
        attempt = 0

        while attempt <= max_retries:
            try:
                logger.debug(f'Попытка подключения к БД #{attempt + 1}')
                ...
                result = client.select(query, params)

                if result:
                    logger.debug('Результат получен')
                    return result
                else:
                    logger.warning('Запрос выполнен, но результат пустой')
                    return None

            except Exception as e:
                attempt += 1
                logger.error(f'Ошибка при работе с БД (попытка {attempt}): {str(e)}')

                if attempt <= max_retries:
                    logger.info(f'Повторная попытка через {retry_delay} сек...')
                    time.sleep(retry_delay)
                else:
                    logger.error(f'Все {max_retries} попыток подключения исчерпаны')
                    return None
    ...
    def RIP_v6_gen(self):
        ...
        with open(f'docs/RIP_v6.xml', 'r') as ex_xml:
            ...
            root[1][0][2][0][1].text = self.quantity
            # BCode
            # print(root[1][0][2][0][9])
            root[1][0][2][0][10][0].text = self.gen_150_mark()

            if self.isMark == False:
                # BCode delete
                position = root[1][0][2][0][10]
                if len(position) > 0 and 'MarkInfo' in position[-1].tag:
                    position.remove(position[-1])
    ...
    def RIP_v4_gen(self):
        ...
        with open(f'docs/RIP_v4.xml', 'r') as ex_xml:
            ...
            root[1][0][2][0][2].text = self.alc_percent_min
            root[1][0][2][0][3].text = self.alc_percent_max
            # Quantity
    ...
    def ticket_get(self, address):
        ...
        try:
            ...
            if comment_element is not None:
                ...
                elif comment_text and 'Использование «старых» алкогольных кодов продукции при фиксации' in comment_text:
                    logger.info("Ошибка старых алккодов ")
                    return ('Получили ошибку про использование "старых" алккодов - считаем, что проверка ФЛК по крепости пройдена', xml_text)
                else:
    ...
    def result_interpretation(self, data_string):
        ...
        productGroup = data.get('GroupProduct', '')
        ...
        if self.alc_percent == self.alc_percent_min:
            addString = 'Крепость равна минимальной'
        elif self.alc_percent == self.alc_percent_max:
            addString = 'Крепость равна максимальной'
        elif int(self.alc_percent) < int(self.alc_percent_min):
            addString = 'Крепость меньше минимальной'
        elif int(self.alc_percent) > int(self.alc_percent_max):
            addString = 'Крепость больше максимальной'
        elif int(self.alc_percent) < int(self.alc_percent_max) and int(self.alc_percent) > int(self.alc_percent_min):
            addString = 'Крепость равна средней'



        report_lines.append(f"# {addString} = {self.alc_percent}, Группа = {productGroup} (CodeVid = {code_vid_egais}, okpd2 = {okpd2})")
...
if __name__ == "__main__":
    ...
    tests = tests_reader("data_of_tests")

    for i in tests:
        age = minAgeSparcle()

        age.alc_percent = i["curAlcPerc"]
        age.alc_percent_min = i["minAlcPerc"]
        age.alc_percent_max = i["maxAlcPerc"]
        ...
        if i["GroupProduct"] == 'ССНП':
            GroupName = "20.42.15.144"
        elif i["GroupProduct"] == 'СЛС':
            GroupName = "20.13.21.120"
        elif i["GroupProduct"] == 'ССПП':
            GroupName = "11.02.12.360"
        else:
            GroupName = "11.02.12.112"
        CodeVid_egais= ''
        update_product = ''
        nsi_data = ''

        while CodeVid_egais == "":
            CodeVid_egais = age.db_selecter("SELECT x.CodeVid_egais FROM EgaisNSI_test.dbo.Product_Vid2021 AS x WHERE CodeVid_classifier = ?",{"0":GroupName})
        # print({"0":CodeVid_egais[0]['CodeVid_egais']})
        while update_product == "":
            update_product = age.db_updater("UPDATE EgaisNSI_test.dbo.Products SET CodeVid_egais = ?, minage = ?, AlcPerc = ?, AlcPercMin = ?, AlcPercMax = ? WHERE Alc_Code = ?",{"0":CodeVid_egais[0]['CodeVid_egais'], "1": i['num_days'], "2":i["alcPerc"], "3":i["minAlcPerc"], "4":i["maxAlcPerc"],"5":age.product_code})
        while nsi_data == "":
