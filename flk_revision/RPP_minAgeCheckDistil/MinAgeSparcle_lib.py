...
class minAgeSparcle():
    ...
    def __init__(self,
        ...
        self.cacheBcode = cacheBcode
        self.yearofGrape = yearofGrape
        self.cacheFB = cacheFB
        self.rip_pCode = rip_pCode
        self.rip_productvid = rip_productvid
    ...
    def db_updater(self, query, params):
        ...
        logger.debug('Подключение успешно')
        logger.debug('Выполняю запрос')
        logger.debug(f'Передаем: (query: {query},params: {params})')
    ...
    def RPP_v4_gen(self):
        ...
        with open(f'docs/RPP_v4.xml', 'r') as ex_xml:
            ...
            # ProductVid
            root[1][0][3][0][1][8].text = self.rip_pCode
            # FB
            root[1][0][3][0][2].text = self.cacheFB
            # MarkInfo
            root[1][0][3][0][4][0].text = self.cacheBcode
    ...
    def RPP_v5_gen(self):
        ...
        with open(f'docs/RPP_v5.xml', 'r') as ex_xml:
            ...
            root[1][0][2][0][5][0].text = self.gen_150_mark()
            # PrVCode
            root[1][0][2][0][6][0][1][8].text = self.rip_productvid
            # FB
            root[1][0][2][0][6][0][2].text = self.cacheFB
            # markinfo
            root[1][0][2][0][6][0][4][0].text = self.cacheBcode
    ...
    def RIP_v6_gen(self):
        ...
        with open(f'docs/RIP_v6.xml', 'r') as ex_xml:
            ...
            # ProductCode
            root[1][0][2][0][0].text = self.rip_pCode
            # Quantity
            root[1][0][2][0][1].text = self.quantity
            # alcPercent
            root[1][0][2][0][2].text = self.alc_percent
            # BCode
            self.cacheBcode = self.gen_150_mark()
            logger.debug(f'Марка: {self.cacheBcode}')
            root[1][0][2][0][10][0].text = self.cacheBcode
            # YearOfGrape
            root[1][0][2][0][9].text = self.yearofGrape
    ...
    def RPP_v6_gen(self):
        ...
        with open(f'docs/RPP_v6.xml', 'r') as ex_xml:
            ...
            # prCodeVid
            root[1][0][2][0][9][0][1][8].text = self.rip_productvid
            # FB
            root[1][0][2][0][9][0][2].text = self.cacheFB
            # markInfo
            root[1][0][2][0][9][0][4][0].text = self.cacheBcode
    ...
    def result_get(self, url_id, docName, offset):
        ...
        while True:
            try:
                ...
                if crypto_ticket_found:
                    ...
                    return self.handle_crypto_ticket_retry(docName)
                    ...
                    while attempt < max_attempts:
                        if url_count > 1:
                            ...
                            for url_element in url_elements:
                                ...
                                url_data.append((url_id, url_text))

                            # Сортируем по ID
                            url_data.sort(key=lambda x: x[0])

                            # Берем второй элемент (индекс 1) - следующий за минимальным
                            if len(url_data) > 1:
                                next_id, next_url = url_data[1-offset]
                                logger.debug(f"Найден следующий ID после минимального: {next_id}, URL: {next_url}")
                                # if 'FORM1REGINFO' in next_url:
                                #     next_id, next_url = url_data[2 - offset]
                                return next_url

                        # Если дошли сюда - следующий тикет не найден
                        logger.debug(f"Попытка {attempt + 1}: следующий тикет не найден, ждем 30 секунд")
                        time.sleep(30)
                        attempt += 1

                    # Если все попытки исчерпаны
                    logger.debug("Все попытки поиска следующего тикета исчерпаны")
                    return self.result_get(url_id, docName, 0)
                else:
    ...
    def handle_crypto_ticket_retry(self, docName):
        ...
        try:
            ...
            if curl_res:
                ...
                return self.result_get(curl_res, docName)
            else:
    ...
    def ticket_get(self, address, exp):
        ...
        try:
            ...
            namespaces = {
                ...
                'wbr': 'http://fsrar.ru/WEGAIS/RepInformF1Reg'
            }

            if 'FORM1REGINFO' in address:
                logger.debug('нашли FORM1REGINFO')
                return (1, 'Документ успешно принят системой на обработку', xml_text)
            ...
            if comment_element is None:
                comment_element = root.find('.//tc:OperationComment', namespaces)
            elif comment_element is None:
                comment_element = root.find('.//wbr:InformF1RegId', namespaces)
            ...
            if comment_element is not None:
                ...
                if 'TEST-FA' in comment_element.text:
                    return (1, 'Документ успешно принят системой на обработку', xml_text)
                elif comment_text and 'Документ успешно принят системой на обработку' in comment_text and exp=='error':
                    logger.info("Документ успешно принят системой на обработку - найдено")
                    return (0)
                elif comment_text and 'Документ успешно принят системой на обработку' in comment_text and exp!='error':
    ...
    def result_interpretation(self, data_string):
        ...
        nsi_data1 = data.get('nsi_data1', '')
        doc_xml = data.get('doc_xml', '')
        xml_rip = data.get('xml_rip', '')
        ticket_xml = data.get('ticket_xml', '')
        expected = data.get('expected', '')
        obtained = data.get('obtained', '')
        RIP_year = data.get('RIP_year','')
        code_vid_egais_rip = data.get('RIPVid', '')

        # Если ticket_xml - это кортеж, берем второй элемент (XML)
        if isinstance(ticket_xml, tuple) and len(ticket_xml) > 1:
            ticket_xml = ticket_xml[2]

        # Формируем строку отчета
        report_lines = []

        # Первая строка с основной информацией
        report_lines.append(f"# minage = {num_days}, фактическая выдержка = {date.today().year}-{RIP_year}={int(date.today().year)-int(RIP_year)}, код вида дистиллята ={code_vid_egais_rip} ,CodeVidEgais отчета о производстве= {code_vid_egais}")

        # Блок NSI_Product (если есть данные)
        if nsi_data:
            report_lines.append("{{collapse(NSI_Product RPP)")
            report_lines.append("<pre>")
            report_lines.append(str(nsi_data))
            report_lines.append("</pre>")
            report_lines.append("}}")
        if nsi_data:
            report_lines.append("{{collapse(NSI_Product сырье)")
            report_lines.append("<pre>")
            report_lines.append(str(nsi_data1))
            report_lines.append("</pre>")
            report_lines.append("}}")

        # Блок doc XML
        report_lines.append("{{collapse(RIP)")
        report_lines.append("<pre>")
        if xml_rip:
            report_lines.append(pretty_xml(xml_rip))
        report_lines.append("</pre>")
        report_lines.append("}}")

        report_lines.append("{{collapse(RPP)")
        report_lines.append("<pre>")
        if doc_xml:
        ...
        # Блок ticket XML
        report_lines.append("{{collapse(ticket RPP)")
        report_lines.append("<pre>")
        if ticket_xml:
...
if __name__ == "__main__":
    ...
    tests = tests_reader("RPP_v4_test2")

    for i in tests:
        age = minAgeSparcle()

        logger.debug('Запускаю предварительную подготовку(RIP)')

        logger.debug('Генерация xml RIP_v6')
        age.yearofGrape = i['RIP_year']
        age.rip_productvid = i['RIPVid']

        logger.debug('Заполняем БД для RIP')

        update_product1 = age.db_updater(
            "UPDATE EgaisNSI_test.dbo.Products SET CodeVid_egais = ?, minage = ? WHERE Alc_Code = ?",
            {"0": i['RIPVid'], "1": i['num_days'], "2": age.rip_pCode})
        nsi_data1 = age.db_selecter(
            "SELECT x.* FROM EgaisNSI_test.dbo.Products AS x WHERE Alc_Code = ?",
            {"0": age.product_code})

        logger.success('Бд для RIP заполнена')


        xml_rip = age.xml_gen_choice('RIP_v6')
        curl_res1 = age.curl_sender_memory(xml_rip)

        if curl_res1:
            address1 = age.result_get(curl_res1, "RIP",0)
            ticket_data1 = age.ticket_get(address1, '')

        i['xml_rip'] = xml_rip

        logger.debug("Поиск формы Б по марке из RIP")

        k=30 # Счетчик если не добавилось ничего в БД

        db_res = ''

        if curl_res1:
            while k!=0:
                logger.debug(f'Поиск, попытка {k}')
                try:
                    connection_db = DB_toonel_old.Connecting()
                    tunnel = connection_db.tooneling(jump_host='10.0.50.208', jump_port=20010, remote_host='10.10.4.172',
                                                     remote_port=5432, username='martikhin',
                                                     ssh_pkey='/home/ivan/.ssh/id_rsa')

                    db_res = connection_db.pg_sql(tunnel,
                                                  'svs',
                                                  'ci',
                                                  'ci',
                                                  f"SELECT code,val FROM public.balance_030000434308 WHERE code = '{age.cacheBcode}'")
                except Exception as e:
                    logger.error(f'Ошибка коннекта: {e}')
                finally:
                    if db_res != []:
                        k=0
                        logger.debug(f'Содержимое результата: {db_res}')
                    else:
                        k-=1
                    time.sleep(1)
        if db_res:
            if db_res!='' and db_res[0][0] == age.cacheBcode:
                age.cacheFB = db_res[0][1]
                ...
                update_product = age.db_updater("UPDATE EgaisNSI_test.dbo.Products SET CodeVid_egais = ?, minage = ? WHERE Alc_Code = ?",{"0":i['CodeVidEgais'], "1": '1', "2":age.product_code})
                ...
                if i['expected'] == 'Успех обработки':
                    exp = ''
                else:
                    exp = 'error'

                if curl_res:
                    ticket_data = (0, 'Ошибка проводки...', '<?xml...>')
                    logger.debug(ticket_data)
                    while ticket_data[0] not in (1, 2, 3):
                        logger.debug(ticket_data)
                        address = age.result_get(curl_res, "RPP", ticket_data[0])
                        ticket_data = age.ticket_get(address, exp)
                        if ticket_data is tuple:
                            pass
                        else:
                            logger.debug(f"Ticket data {ticket_data}")

                    i['ticket_xml'] = ticket_data
                    i['obtained'] = ticket_data[1]

                age.result_interpretation(i)

            else:
                logger.error('Произошла проблема в подготовке для тестирования')
