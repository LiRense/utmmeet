...
class minAgeSparcle():
        ...
        try:
            ...
            if comment_element is not None:
                ...
                if comment_text and 'Документ успешно принят системой на обработку' in comment_text:
                    ...
                    return ('Получили ошибку про использование "старых" алккодов - УСПЕХ', xml_text)
                else:
    ...
    def result_interpretation(self, data_string):
        ...
        report_lines.append(f"# Группа = {productGroup}, CodeVid = {code_vid_egais}")
...
if __name__ == "__main__":
    ...
    for i in tests:
        ...
        while update_product == "":
            update_product = age.db_updater("UPDATE EgaisNSI_test.dbo.Products SET CodeVid_egais = ?, minage = ?, AlcPerc = ?, AlcPercMin = ?, AlcPercMax = ? WHERE Alc_Code = ?",{"0":int(CodeVid_egais), "1": i['num_days'], "2":i["alcPerc"], "3":i["minAlcPerc"], "4":i["maxAlcPerc"],"5":age.product_code})
        while nsi_data == "":
