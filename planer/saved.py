    ...
    try:
        ...
        if user_id in user_groups and groups[user_groups[user_id]]['owner'] == user_id:
            ...
            for i, member_id in enumerate(group['members'], 1):
                member_id = str(member_id)
                username = user_names.get(member_id, f"Пользователь {member_id}")
                text += f"{i}. {username}\n"
...
def process_group_code(message, temp_msg_id):
    try:
        user_id = str(message.from_user.id)
        group_code = message.text.upper().strip()
        ...
        if group_id:
            ...
            # Сохраняем связь пользователя с группой
            user_groups[user_id] = group_id
            save_data()

            bot.send_message(
                message.chat.id,
                f"Вы успешно подключились к группе {group_code}!",
