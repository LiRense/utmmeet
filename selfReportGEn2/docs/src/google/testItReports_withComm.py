...
class TestReport:
    """
    Класс для формирования отчетов о выполнении тестов в формате Json.
    ...
    .. code-block:: json
       ...
       {
           "configurationId": "uuid-example-12345",
           "autoTestExternalId": "test-001",
           "outcome": "Passed",
           "message": "Тест выполнен успешно",
           "traces": "Нет трассировок ошибок",
           "startedOn": "2025-08-13T12:00:00Z",
           "completedOn": "2025-08-13T12:05:00Z",
           "duration": 300000,
           "parameters": {
               "env": "production",
               "timeout": 60
    ...
    .. list-table:: Описание полей
       ...
       * - ``configurationId``
         - Уникальный идентификатор конфигурации (UUID).
       * - ``autoTestExternalId``
         - Внешний идентификатор теста (строка).
       * - ``outcome``
         - Общий результат выполнения теста. Возможные значения: ``Passed``, ``Failed``, ``Skipped``.
       * - ``message``
         - Сводное сообщение о результате выполнения теста (строка).
       * - ``traces``
         - Трассировки ошибок или лог выполнения (строка).
       * - ``startedOn``
         - Время начала выполнения теста в формате ISO8601.
       * - ``completedOn``
         - Время окончания выполнения теста в формате ISO8601.
       * - ``duration``
         - Длительность выполнения теста в миллисекундах (целое число).
       * - ``parameters``
         - Глобальные параметры теста в формате ``{"key": "value"}``.
       * - ``properties``
         - Дополнительные свойства теста в формате ``{"key": "value"}``.
       * - ``stepResults``
         - Иерархический список результатов шагов теста. Каждый шаг содержит ``title``, ``description``, ``outcome`` и вложенные ``stepResults``.
       * - ``setupResults``
         - Результаты выполнения этапа подготовки (setup) в формате списка.
       * - ``teardownResults``
         - Результаты выполнения этапа завершения (teardown) в формате списка.
