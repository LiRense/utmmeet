    ...
    class StepResults:
        ...
        def skip(self, reason: str = ""):
            ...
            if self._step_stack:
                ...
                # Устанавливаем флаг пропуска только для подшагов текущего шага
                self._skip_remaining = True
            elif self._current_test:
                # Пропускаем текущий тест
                self._current_test.update({
                    "outcome": "Skipped",
                    "message": reason or "Тест пропущен",
                    "completedOn": datetime.utcnow().isoformat() + "Z",
                    "traces": ""
                })
                duration = (datetime.fromisoformat(self._current_test["completedOn"][:-1]) -
                            datetime.fromisoformat(self._current_test["startedOn"][:-1]))
                self._current_test["duration"] = int(duration.total_seconds() * 1000)
                # Устанавливаем флаг пропуска только для шагов текущего теста
                self._skip_remaining = True
        ...
        def info(self, info: str):
            if self._current_test:
                self._current_test["info"] = info

        def parameter(self, key: str, value: str):
            if self._current_test:
                self._current_test["parameters"][key] = value
