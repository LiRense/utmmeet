...
class TestReport:
    ...
    Основное применение:
    - Интеграция с системами тест-менеджмента (testIt)
    - Автоматизированное тестирование с детальной отчетностью
    - Комплексные тестовые сценарии с вложенными шагами

    Attributes:
        _configuration_id (str): Уникальный идентификатор конфигурации.
            Формат: UUID4. Автоматически генерируется при создании экземпляра.
        _auto_test_external_id (str): Внешний ID теста (из testIt).
            По умолчанию: "default_test_id".
        _parameters (Dict[str, str]): Глобальные параметры тестирования.
            Пример: {"environment": "prod", "browser": "chrome"}
        _properties (Dict[str, str]): Метаданные тестового прогона.
            Пример: {"version": "1.0.0", "team": "QA Automation"}
        _step_results (List[Dict]): Результаты выполнения шагов тестов.
            Хранит иерархическую структуру тестовых шагов.
        _setup_results (List[Dict]): Результаты методов setup.
        _teardown_results (List[Dict]): Результаты методов teardown.
        _global_start_time (Optional[datetime]): Время начала прогона (UTC).
        _global_end_time (Optional[datetime]): Время окончания прогона (UTC).
        _global_outcome (str): бщий результат прогона ("Passed"/"Failed"/"Skipped").
        _global_message (str): Сводное сообщение о результатах.
        _global_traces (str): Агрегированные трассировки ошибок (если есть).
        _report_filename (str): Путь к файлу для сохранения отчета.
            По умолчанию: "test_report.json" в текущей директории.
        _debug (bool): Режим отладки (логирование в stderr при True).
    ...
    def configure(self,
        ...
        Args:
            configuration_id (Optional[str]): Уникальный идентификатор конфигурации.
            auto_test_external_id (Optional[str]): Внешний идентификатор теста.
            report_filename (Optional[str]): Имя файла для сохранения отчета.
            debug (bool): Флаг отладки (вывод дополнительной информации).

        Raises:
            ValueError: Если переданные параметры имеют недопустимый формат.
        """
        if configuration_id:
    ...
    def parameter(self, key: str, value: str):
        ...
        Raises:
            TypeError: Если ключ или значение не являются строками.
        """
    ...
    def end_test_run(self):
       ...
       Returns:
           Dict: Сформированный отчет о тестировании.

       Raises:
           RuntimeError: Если тестовый прогон не был должным образом начат или завершен.
       """
    ...
    def _save_report(self, report: Dict):
        ...
        Raises:
            ...
            PermissionError: Если нет прав на запись в указанное место.
            TypeError: Если отчет имеет неверный формат.
        """
        try:
    ...
    def run_tests(self, test_functions: List[Callable], setup_teardown_func: Optional[Callable] = None):
        ...
        Args:
            test_functions (List[Callable]): Список тестовых функций для выполнения.
            setup_teardown_func (Optional[Callable]): Функция-фикстура для setup/teardown.
    ...
    class StepResults:
        ...
        def __init__(self, report):
            ...
            Args:
                report (TestReport): Родительский объект TestReport.
            """
            self._report = report
        ...
        def _get_current_step(self):
            ...
            Returns:
                Optional[Dict]: Текущий активный шаг или тест, или None если нет активных шагов.
            """
        ...
        def title(self, title: str):
            ...
            Returns:
                Callable: Декоратор функции теста.
            """
        ...
        def step(self, title: str):
            ...
            Returns:
                Callable: Декоратор функции шага.

            Raises:
                RuntimeError: Если шаг создается вне контекста теста.
                ValueError: Если название шага пустое.
            """
        ...
        def _handle_error(self, error_msg: str):
            ...
            Raises:
                RuntimeError: Если нет активного шага/теста для обработки ошибки.
            """
        ...
        def description(self, description: str):
            """Устанавливает описание для текущего шага или теста.
             ...
             Args:
                 description (str): Описание шага или теста.
             """
            current_step = self._get_current_step()
            if current_step:
    ...
    class SetupResults:
        ...
        def add(self, title: str, description: str = "", info: str = ""):
           ...
           Args:
               title (str): Название setup-метода.
               description (str): Описание setup-метода.
               info (str): Дополнительная информация.

           Returns:
               Dict: Добавленный setup-метод.
        """
            setup_data = {
                "title": title,
        ...
        def parameter(self, key: str, value: str):
            ...
            Raises:
                ValueError: Если название пустое или параметры имеют неверный формат.
            """
