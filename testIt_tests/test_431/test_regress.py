...
def repproduceproduct_v4():
    ...
    def enviroment_check():
        nonlocal actions_ids
        try:
            db_selecter = DB_placer(config['fixationDB']['db_name'],
                                   config['fixationDB']['db_user'],
                                   config['fixationDB']['db_password'],
                                   config['fixationDB']['db_host'],
                                   config['fixationDB']['db_port'])
            db_selecter.sql_req = f"SELECT id, check_code FROM checks.documents_checks dc WHERE document_name_format = '{doc_type}' AND service_code = '{config['rules']['rule_user']}'"
            result = db_selecter.inserter()
            if result:
                testReport.stepResults.description(f"Для документа найдены проверки: {result}")
                actions_ids = result
                assert True, f"Все action найдены"
            else:
                testReport.stepResults.description("Для документа не найдены правила")
                assert False, f"Для документа не найдены правила: {result}"
        except Exception as e:
            logger.error(f'Ошибка при проверке правил: {e}')
            assert False, f"Ошибка при проверке правил: {e}"
    ...
    def rpp_4_doubles():
        ...
        def rpp_4_insert_db():
            ...
            def doc_insert():
                ...
                try:
                    db_ins = DB_placer(config['transportDB']['db_name'],
                                   config['transportDB']['db_user'],
                                   config['transportDB']['db_password'],
                                   config['transportDB']['db_host'],
                                   config['transportDB']['db_port'])
            ...
            def product_insert():
                ...
                try:
                    db_ins = DB_placer(config['nsiDB']['db_name'],
                                   config['nsiDB']['db_user'],
                                   config['nsiDB']['db_password'],
                                   config['nsiDB']['db_host'],
                                   config['nsiDB']['db_port'])
            ...
            def registry_insert():
                ...
                try:
                    db_ins = DB_placer(config['documentsDB']['db_name'],
                                   config['documentsDB']['db_user'],
                                   config['documentsDB']['db_password'],
                                   config['documentsDB']['db_host'],
                                   config['documentsDB']['db_port'])
        ...
        def rpp_4_json_send():
            ...
            try:
                sender = Kafka_sender(forming.doc_type, params[1], topic=config['kafka']['topic'], bootstrap_server=config['kafka']['bootstrap_servers'])
                sender.send()
